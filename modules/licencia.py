...
def _cache_path() -> str:
    ...
    d = os.path.join(base, 'DeepCore Agenda Pro')
...
def _llave_guardada_path() -> str:
    base = os.environ.get('APPDATA', os.path.expanduser('~'))
    return os.path.join(base, 'DeepCore Agenda Pro', 'key.dat')
...
def validar_online(key: str) -> dict | None:
    try:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': f'DeepCore-{_PROG_ID}/2.0'})
...
def _validar_formato(key: str) -> tuple:
    ...
    try:
        ...
        hoy = date.today()
        if hoy < inicio:
            return False, 0, f"Licencia válida desde: {inicio.strftime('%d/%m/%Y')}"
        if hoy >= fin:
            ...
            return False, -dias_venc, "Licencia expirada. Contacta a DeepCore para renovar."
...
def estado_licencia(key: str) -> dict:
    ...
    if data:
        ...
        return {'valida': valida, 'dias': dias, 'mensaje': msg,
                'color': '#10B981' if valida else '#EF4444',
                'advertir': valida and dias < 14, 'hardware_id': hw, 'fuente': 'online'}
    cache = _leer_cache(key)
    if cache:
        valida = cache.get('valida', False)
        return {'valida': valida, 'dias': cache.get('dias', 0),
    ...
    return {'valida': valida, 'dias': dias, 'mensaje': msg + ' [SIN CONEXIÓN]',
            'color': '#F59E0B' if valida else '#EF4444',
